...
- `~/.codex/azure-astra.config.toml` is an explicit Azure Astra overlay. Credentials are generated into native `~/.codex/.env` from the existing local canonical Azure key; see [Azure Astra setup](codex-azure-astra.md) for materialization, validation, and desktop activation.
