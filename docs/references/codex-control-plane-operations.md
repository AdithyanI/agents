...
- `~/.codex/azure-astra.config.toml` is an explicit Azure Astra overlay using Codex's default model metadata and protocol. Credentials are generated into native `~/.codex/.env` from the existing local canonical Azure key; see [Azure Astra setup](codex-azure-astra.md) for configuration, validation, and app restart instructions.
