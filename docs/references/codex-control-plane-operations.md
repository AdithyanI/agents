...
- `~/.codex/azure-astra.config.toml` is an explicit Azure Astra overlay using Codex's default model metadata and protocol. The September 18, 2026 rollback removed the generated standard-Responses catalog and subscription snapshot override. Both providers now use default model discovery; sync clears the old overrides and deletes its retired Azure catalog. Credentials are generated into native `~/.codex/.env` from the existing local canonical Azure key; see [Azure Astra setup](codex-azure-astra.md#web-search-in-the-azure-profile) for the current contract, historical search limitation, and app restart instructions.
