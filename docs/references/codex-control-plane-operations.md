...
- `~/.codex/auth.json` retains the ChatGPT login. As requested on 2026-09-16, [`global.config.toml`](/Users/dobby/GitHub/agents/codex/config/global.config.toml) selects `model_provider = "azure"` and `model = "gpt-6-astra"`; use `codex --profile chatgpt` for subscription access. Reasoning effort and `service_tier` remain client-owned. See [Azure Astra setup](codex-azure-astra.md) for app restart and history considerations.
