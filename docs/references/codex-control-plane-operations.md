...
- `~/.codex/auth.json` retains the ChatGPT login. As requested on 2026-09-16, [`global.config.toml`](/Users/dobby/GitHub/agents/codex/config/global.config.toml) selects `model_provider = "azure"` and `model = "gpt-6-astra"`; use `codex-openai` (or `codex --profile chatgpt`) for subscription access. The `chatgpt` profile requires ChatGPT authentication; `codex-azure` selects Azure explicitly. Reasoning effort and `service_tier` remain client-owned. See [Azure Astra setup](codex-azure-astra.md) for launcher setup, app restart, and history considerations.
...
- `~/.codex/azure-astra.config.toml` is an explicit Azure Astra overlay. It loads a machine-local catalog derived by `codex/scripts/sync-azure-model-catalog.py` during config sync, changing only Astra to standard Responses for Azure-hosted web search. The shared global/desktop default now loads that same Azure catalog. The subscription profile explicitly restores `models_cache.json` and standalone search; explicit catalogs require the documented refresh procedure. Credentials are generated into native `~/.codex/.env` from the existing local canonical Azure key; see [Azure Astra setup](codex-azure-astra.md#web-search-in-the-azure-profile) for the rationale, refresh, validation, rollback, and app restart instructions.
