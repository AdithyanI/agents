...
- `~/.codex/azure-astra.config.toml` is an optional Azure Astra overlay. The registered `azure` provider does not change the subscription default. Credentials are generated into native `~/.codex/.env` from the existing local canonical Azure key; see [Azure Astra setup](codex-azure-astra.md) for materialization, validation, and desktop activation.
